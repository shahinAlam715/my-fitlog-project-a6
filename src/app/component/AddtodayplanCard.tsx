...
const AddtodayplanCard = ({data}:{data: Ifitlog}) => {
    ...
    const handleaddplan = ()=>{
        if (isAdded) {
            return(
                toast.warn(`${data.name} is Allrady Added`)
            )
        }
    ...
    return (
        <div>
            ...
            <span className='flex gap-4 justify-center items-center'><i><IoBagAdd /></i>{isAdded ? "Added to plan" :"Add to today,s plan"}</span>
            </button>
        </div>
    );
};
