...
const Savecard = ({ allSaveplan }: { allSaveplan: Ifitlog[] }) => {
  const { saveplan, setSaveplan } = useContext(FitContext);

  const handleremove = (plan: Ifitlog) => {
    const removeData = saveplan.filter((item) => item.id !== plan.id);
    setSaveplan(removeData);
    toast.warning(`Save Plan is remove !`);
  };
  ...
  return (
    <div>
      {allSaveplan.map((plan: Ifitlog, i: number) => (
        <div
          key={i}
          className="border-b border-gray-700 my-4 bg-[#18181B] px-8 py-3 grid grid-cols-1 lg:grid-cols-2 rounded-xl"
        >
